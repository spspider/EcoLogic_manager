...
#define NUM_LEDS    52//40
CRGB leds[NUM_LEDS];

//CRGB leds_prep[NUM_LEDS];
#define NUM_CHARTS    2//2
...
void load_pattern()
{
  ...
  for (char i = 0; i < num_ws8211; i++) {
    ...
    switch (type_ws8211[i]) {
      ...
      case 1://с двух сторон
        // case 5:
        pos[i] = reverseDirection(i);
        //leds[i] += CHSV( gHue, 255, 192);//бегающий огонь
        leds[i] = CHSV( gHue, 255, 192);//бегающий огонь
        //leds[i] += CHSV( gHue, 255, 192);//бегающий огонь
        unsigned char  calc_pos;
        //calc_pos = (from_ws8211[i] - pos[i]) > to_ws8211[i] ? to_ws8211[i] : (from_ws8211[i] - pos[i]) ;
        //calc_pos = dir_ws8211[i] == 0 ? (to_ws8211[i] - pos[i] + from_ws8211[i]) : (to_ws8211[i] - pos[i] + from_ws8211[i]) ;
        calc_pos = (to_ws8211[i] - pos[i] + from_ws8211[i]);
        leds[calc_pos] = CHSV( gHue, 255, 192); //бегающий огонь
        break;
      case 2://полностью в цвете
        for (char i1 = from_ws8211[i]; i1 < to_ws8211[i]; i1++) {
          leds[i1] = CHSV(  gHue, 255, 192);//простая линия света
        }
        break;
      case 3://середина в цвете
        for (char i1 = from_ws8211[i]; i1 < to_ws8211[i]; i1++) {
          if (i1 < mid) {
            leds[i1] = CHSV(  gHue, 255, maxLight * i1);
          }
          else {
            leds[i1] = CHSV(  gHue, 255, maxLight * (to_ws8211[i] - i1));
          }
        }
        break;
      case 4://не работает
        leds[random(from_ws8211[i], to_ws8211[i])] += CHSV( gHue, 255, 192); //бегающий огонь
        break;
      case 5://мигание через одну
        ...
        for (char i1 = from_ws8211[i]; i1 < to_ws8211[i]; i1++) {
          if (pos[i] % 2 == 0) {
            if ( (i1 % 2) == 0) {
              leds[i1] = CHSV( gHue, 255, 192);//бегающий огонь
            }
          } else {
            if ( (i1 % 2) != 0) {
              leds[i1] = CHSV( gHue, 255, 192);//бегающий огонь
  ...
  //leds[num_leds_fade - pos] += CHSV( gHue, 255, 192);
}
...
void loop_ws2811()
{
  ...
  if (millis() > (sp_ws8211 * 16) + millis_strart) {
    for (char i = 0; i < num_ws8211; i++) {
      if (pos[0] == 0) {
        //check_for_changes();
      }

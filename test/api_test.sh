...
DEVICE_ID="esp2A006C_BCDDC22A006C"
TOKEN="tk01"
...
curl -X POST "$BASE_URL/api/cfg?id=$DEVICE_ID&tk=$TOKEN" \
...
curl -X GET "$BASE_URL/api/config?id=$DEVICE_ID"

echo -e "\n"

# 3. Update real status from Arduino
echo "3. Update real status (/api/real)"
curl -X POST "$BASE_URL/api/real?id=$DEVICE_ID&tk=$TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "stat": ["0", "1", "23.5"]
  }'
...
curl -X GET "$BASE_URL/api/desired?id=$DEVICE_ID&tk=$TOKEN"

echo -e "\n"

# 5. AJAX request - get status (t=127)
echo "5. AJAX get status (/api/ajax)"
curl -X GET "$BASE_URL/api/ajax?device_id=$DEVICE_ID&data=%7B%22t%22%3A127%2C%22v%22%3A0%7D"

echo -e "\n"

# 6. AJAX request - set pin value (t=0, v=1)
echo "6. AJAX set pin 0 to value 1 (/api/ajax)"
curl -X GET "$BASE_URL/api/ajax?device_id=$DEVICE_ID&data=%7B%22t%22%3A0%2C%22v%22%3A1%7D"
...
curl -X GET "$BASE_URL/api/home?device_id=$DEVICE_ID"

echo -e "\n"

# 9. Get device selector page
echo "9. Get device selector (/api/device_selector)"
curl -X GET "$BASE_URL/api/device_selector"
